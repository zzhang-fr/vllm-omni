...
Usage:
    # SpargeAttn backend (auto-detected from installed plugins)
    python wan2_2_sparse.py --attn-backend auto

    # With custom top-k ratio (lower = faster, less accurate)
    python wan2_2_sparse.py --attn-backend spargeattn --sparse-attn-topk 0.3

    # JSON config (matches serve.py --attn-backend syntax)
    python wan2_2_sparse.py --attn-backend '{"backend":"spargeattn","topk_ratio":0.3}'

    # Compare with dense baseline (no sparse attention)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Sparse attention plugin. Simple name: spargeattn, dense. "
        "JSON: '{\"backend\":\"spargeattn\",\"topk_ratio\":0.3}'. "
        "Import path: 'module.path:func_name'. "
        "When a sparse plugin is specified, sparse_attention dispatcher "
        "is auto-selected.",
    )
    parser.add_argument(
        "--sparse-attn-topk",
        type=float,
        default=0.5,
        help="Top-k ratio for sparse attention (0-1). Default: 0.5.",
    )
...
def main():
    ...
    # Build sparse attention config from --attn-backend + --sparse-attn-topk
    attn_backend_name = args.attn_backend
    sparse_attn = None
    if attn_backend_name:
        import json as _json
        if attn_backend_name.strip().startswith("{"):
            cfg = _json.loads(attn_backend_name)
            cfg.setdefault("topk_ratio", args.sparse_attn_topk)
            sparse_attn = DiffusionSparseAttnConfig.from_dict(cfg)
            attn_backend_name = None  # engine infers from sparse_attn
        else:
            sparse_attn = DiffusionSparseAttnConfig(
                backend=attn_backend_name,
                topk_ratio=args.sparse_attn_topk,
            )
            attn_backend_name = None  # engine infers from sparse_attn
    ...
    topk_str = f"{sparse_attn.topk_ratio}" if sparse_attn else "N/A"
    print(f"\n{'=' * 60}")
    print("Wan 2.2 Attention Configuration:")
    print(f"  Backend: {backend_str}")
    print(f"  Top-k ratio: {topk_str}")
